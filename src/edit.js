...
const edit = ({ attributes, setAttributes, clientId }) => {
    ...
    useEffect(() => {
        ...
        if (backgroundImage.source !== 'featured') return;

        // Si se borra la featured
        if (!featuredImageId) {
            setAttributes({ backgroundImage: null });
            return;
        }

        // Si se reemplaza la featured
        if (featuredImage && backgroundImage.id !== featuredImage.id) {
            setAttributes({
                backgroundImage: {
                    ...backgroundImage,
                    id: featuredImage.id,
    ...
    }, [featuredImageId, featuredImage, backgroundImage]);
